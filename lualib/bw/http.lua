...
function M.get(url, get, header, no_reply)
    if no_reply then
        return skynet.send(sname.WEBCLIENT, "lua", "request", url, get, nil, header, no_reply)
    else
        return skynet.call(sname.WEBCLIENT, "lua", "request", url, get, nil, header, no_reply)
    end
end
...
function M.post(url, post, header, no_reply)
    ...
    if no_reply then
        ...
        return skynet.call(sname.WEBCLIENT, "lua", "request", url, nil, post, header, no_reply)
    end
end
