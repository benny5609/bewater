...
return function()
    ...
    return {
        list = {
            ...
            {k="性能", v=profile and string.format("CPU:%sMEM:%.fM", profile.cpu, profile.mem/1024) or "未知"},
            {k="监听端口", v=conf.gate and string.format("%s:%s", conf.gate.host, conf.gate.port) or "未知"},
            {k="后台地址", v=conf.webconsole and string.format("%s:%s", conf.webconsole.host, conf.webconsole.port) or "未知"},
            {k="mongo", v=conf.mongo and string.format("%s:%s[%s]", conf.mongo.host, conf.mongo.port, conf.mongo.name) or "未知"},
