...
function M:remove_player(uid)
    ...
    db.delete("operate", {uid = uid})
end

function M:get_player(uid)
    local player = self.players[uid]
    if not player then
        local data = db.find_one_with_default("operate", {uid = uid}, {
            uid = uid,
            time = os.time(),
            operate_list = {},
        }) 
...
function M:operate(uid, code, ...)
    ...
    local player = self:get_player(uid)
    trace("%s, %s, %s", player.cname, player.agent, opcode.toname(code))
    if player.agent then
        --skynet.try(function()
            cluster.call(player.cname, player.agent, "operate", "operate", code, table.unpack(params))
            player.time = os.time()
            self:save_player(uid)
        --end)
        return
